...
--chekckbox-checked-hover-background: var(--color-selectioncontrols-selection-hover); // some components has checked state, it will be added before <state> because checked is more static than temporary states like hover.
```
...
``` scss
@import "@infermedica/component-library/styles/mixins/mixins";
...
More about our CSS Variable and our approach to it you can find in [CSS Variables Deep Deving](docs/contributing-guide-css-var-deep-dive--page).
...
your own component `MyAwesomeButton` with class `my-awsome-button` on it and use it instead `UiButton` directly.

```html
<template>
  <UiButton class="my-awsome-button">
    <slot/>
  <UiButton>
</template>

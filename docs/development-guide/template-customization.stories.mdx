...
## Attrs props
Nested components can be setup by using `Attrs` props i.e. you can pass props to UiButton, UiText etc. inside more complex components.
For example from `UiNumberStepper` component:

```html
<template>
  <UiButton
    v-bind="decrementButtonAttrs"
  >
    ...
  </UiButton>
</template>
...
Generally the name this props we used `<element-name><name-of-component-without-prefix>Attrs` e.g. `closeButtonAttrs` or `decrementButtonAttrs`.
